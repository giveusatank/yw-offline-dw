...
object DwdActionDoLog2DwsDevice {
  def writeActionDoLog2DwsUvSessionDaily(spark: SparkSession, yestStr: String): Unit = {
    spark.sql("use dws")
    val createSql =
      """
        |create table if not exists dws_device_session_daily(
        |product_id string,
        |company string,
        |remote_addr string,
        |country string,
    ...
    val _sql1 =
      s"""
         |insert overwrite table dws.dws_device_session_daily partition (count_date)
         |select
         |a.product_id,
         |a.company,
         |a.remote_addr,
         ...
         |group by product_id, dws.yunwangDateFormat('company',company), active_user, group_id, put_date) a
  ...
  def writeActionDoLog2DwsUvDaily(spark: SparkSession, yestStr: String): Unit = {
    ...
    val createSql =
      """
        |create table if not exists dws_device_daily(
        |product_id string,
        |company string,
        |remote_addr string,
        |country string,
    ...
    val _sql1 =
      s"""
         |insert overwrite table dws.dws_device_daily partition (count_date)
         |select product_id,
         |       company,
         ...
         |       sum(launch_count),
         |       count_date
         |from dws.dws_device_session_daily
  ...
  /**
    ...
    * @param _7DaysBefore
    */
  def writeDwsUvDaily2DwsUvTotal(spark: SparkSession, yestStr: String): Unit = {
    spark.sql("use dws")

    val createSql =
      """
        |create table if not exists dws_device_total(
        |product_id string,
        |company string,
        |remote_addr string,
        |country string,
    ...
    val clearSql =
      s"""
         |insert overwrite table dws.dws_device_total
         |select product_id,
         |       company,
  ...
  def writeDwsUvDaily2DwsUvIncrease(spark: SparkSession, yestStr: String): Unit = {
    ...
    val createSql =
      """
        |create table if not exists dws_device_increase(
        |product_id string,
        |company string,
        |remote_addr string,
        |country string,
    ...
    val insertSql =
      s"""
         |insert overwrite table dws.dws_device_increase partition (count_date)
         |select product_id,
         |       company,
         |       remote_addr,
         |       country,
  ...
  def main(args: Array[String]): Unit = {
    ...
    breakable {
      ...
      for (i <- 0 until (if (args.length > 0) args.length else 1)) {
        ...
        //执行业务逻辑
        action(spark, yestStr, todayStr)
      }
    }
    spark.stop()
  }

  def action(spark: SparkSession, yestStr: String, todayStr: String): Unit = {
    ...
    writeDwsUvDaily2DwsUvTotal(spark, yestStr)
...
select product_id,
       ...
       sum(launch_used_time)                                          as launch_used_time,
       sum(launch_count)                                              as launch_count,
       '20200923' as put_date
from dws.dws_device_daily
group by product_id, company, active_user, device_id) a
left join (select * from dwd.dwd_product_user where org_id is not null) b
on a.active_user=b.user_id and a.product_id=b.product_id and a.company=b.company
left join dim.dim_organization c on b.org_id=c.id;
   */
